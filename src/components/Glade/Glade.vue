...
.card-place-header{margin:0 20px 0 10px}
...
.coords-block{padding:8px 12px}
.coords-block:hover{background:rgba(138,181,57,.08)}
.coords-value{
...
.card-place:hover .card-footer{transform:scale(1.1)}
.card-place:hover .map-open{text-decoration:underline}

.marker{display:inline-block;animation:bounce2 2s cubic-bezier(.4,0,.2,1) infinite}

.marker-icon{margin-top:-6px}
...
.block-long{flex:2}
.block-small{flex:1;text-align:center;cursor:pointer}
...
/* animations */
@keyframes sway{0%,100%{transform:rotate(0)}25%{transform:rotate(4deg) translateY(-2px)}75%{transform:rotate(-4deg) translateY(2px)}}
@keyframes rotY{0%,100%{transform:rotateY(0)}25%{transform:rotateY(90deg)}50%{transform:rotateY(180deg)}}
